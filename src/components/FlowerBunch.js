...
export function FlowerBunch(props) {
    const { bunchName, flowers, viewFlower, initialOpenStatus, descriptionsHidden, bgColor } = props;
    ...
    const numberClassName = `px-1 min-w-[16px] text-xs font-bold rounded-full text-white ${newFlowers.length > 0 ? 'bg-pink-400' : 'bg-lime-500'}`
        ...
        <div>
            <div className="px-2 flex justify-between py-2 space-x-2 items-center hover:cursor-pointer max-w-[750px] m-auto" style={{ backgroundColor: getLightHexCode(bgColor) }}>
                <div className="flex space-x-1.5 items-center">
                    <div>{bunchName}</div>
                    <div className={numberClassName}>{numFlowers}</div>
